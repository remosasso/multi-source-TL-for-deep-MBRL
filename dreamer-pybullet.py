...
class Dreamer(tools.Module):
  ...
  def __call__(self, obs, reset, state=None, training=True):
    ...
    #  prev_loss = self._imgloss
      with self._strategy.scope():
        for train_step in range(n):
          log_images = self._c.log_images and log and train_step == 0
          self.train(next(self._dataset), log_images)
          
     # if prev_loss.numpy() == self._imgloss.numpy():
      #    self._losscount +=1
      #else:
       #   self._losscount = 0
      #print("Losscount: " + str(self._losscount))
     # if self._losscount == 10:
      #    self.save_single(self._encode, "/data/s2965917/dreamer/dreamer/stored_encoder/encoder.pkl")
       #   exit(1)
