...
def main(args):
  rows = 1#int(np.ceil(len(tasks) / args.cols))
  args.cols = 3
  figsize = args.size[0] * args.cols, args.size[1] * rows
  ...
  path = args.indir[0]
  titles = ['2 tasks', '3 tasks', '4 tasks']
  for idx, i in enumerate(["hoppend","hoppendwalk","hoppendwalkant"]):
    args.indir[0] = path / ("meta-doublepend-"+i)
    print(args.indir[0])
    find_keys(args)
    print(args.tasks)
    ...
    for run in runs:
      if len(run.ys) == 995:
        temp = np.append(run.ys,[run.ys[993],run.ys[994]])
      ...
      #  print(tenperc)
        ...
        print([run.ys[982],run.ys[983],run.ys[984],run.ys[985],run.ys[986],run.ys[987],run.ys[989]])
        temp = np.append(run.ys,[run.ys[982],run.ys[982],run.ys[982],run.ys[982],run.ys[982],run.ys[982],run.ys[982]])
        full.append(temp[0:len(temp)])
        tenperc = len(temp) - int((len(temp)/100*10))
       # print(len(temp))
      #  print(tenperc)
    ...
    for task, ax in zip(tasks, axes.flatten()):
      fig = figure(runs, args, singles, fig, task, rows,axes.flatten()[idx],titles[idx])
