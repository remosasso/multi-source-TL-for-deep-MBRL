...
class Dreamer(tools.Module):
  ...
  def _train(self, data, log_images):
    with tf.GradientTape() as model_tape: #model gradient
      ...
      reward_pred = self._reward(feat) #predict reward for st+1, so rt+1
      likes = tools.AttrDict()
      likes.image = tf.reduce_mean(image_pred.log_prob(data['image'])) #decoder loss
    ...
    with tf.GradientTape() as actor_tape: #actor gradient
      imag_feat = self._imagine_ahead(post) #get st+2
      reward = self._reward(imag_feat).mode() #get rt+2
      if self._c.pcont:
...
def main(config):
  print(config.logdir)
  print("FACTOR BELOW HERE")
  print(config.transfer_factor)
  ...
  if config.transfer:
      if (config.logdir.parent.parent / 'variables.pkl').exists():
        print('Load transfer checkpoint.')
        agent.load(config.logdir.parent.parent / 'variables.pkl')
      else:
  ...
  for var in agent._encode.variables:
    print(var)
    break

  agent.load_single() #load pretrained encoder
  agent._encode.freeze() #freeze encoder
  ...
  agent.save_single(agent._reward,'/data/s2965917/dreamer/dreamer/stored_rews_ae/'+config.task1+'.pkl') #save reward model seperately
